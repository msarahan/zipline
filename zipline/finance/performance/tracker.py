...
class PerformanceTracker(object):
    ...
    def __init__(self, sim_params, trading_calendar, env):
        ...
        first_open = self.sim_params.first_open.tz_convert(trading_calendar.tz)

        self.current_session = pd.Timestamp(
            datetime(
                first_open.year,
                first_open.month,
                first_open.day
        ...
        if self.emission_rate == 'daily':
            self.all_benchmark_returns = pd.Series(index=self.sessions)
