...
from zipline.utils.calendars.trading_calendar import default_nyse_calendar
...
""".strip().format(class_name=self.__class__.__name__,
        ...
        if isinstance(data, DataPortal):
            ...
            if isinstance(data, pd.Panel):
                ...
                if overwrite_sim_params:
                    self.sim_params = self.sim_params.create_from_copy(
                        data.major_axis[0],
                        data.major_axis[1]
                    )
    ...
    def _run_pipeline(self, pipeline, start_session, chunksize):
        ...
        sim_end_session = self.sim_params.period_end
